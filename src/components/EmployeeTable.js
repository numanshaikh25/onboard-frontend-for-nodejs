...
function EmployeeTable() {
  ...
  useEffect(() => {
    ...
    async function fetchEmployees() {
      const { data } = await axios.get(
        "https://onboard-backend-nodejs.herokuapp.com/api/employee/getemployees",
        config
      );
  ...
  const deleteHandler = (id) => {
    ...
    if (window.confirm("Are you sure you want to delete this emloyee?")) {
      axios
        .delete(
          `https://onboard-backend-nodejs.herokuapp.com/api/employee/deleteemployee/${id}`,
          config
        )
        .then((res) => {
          setLoading(false);
          ...
          <tbody>
            {employees.map((employee, index) => (
              <tr key={employee._id}>
                ...
                <td>
                  <Link
                    to={`/employeedetail/${employee._id}`}
